...
RUN python -m pip install --index-url https://download.pytorch.org/whl/cpu \
   ...
   && python -m pip install fastapi uvicorn[standard] opencv-python-headless             ffmpeg-python moviepy librosa numpy scipy tqdm huggingface_hub gdown      && python -m pip install --index-url https://download.pytorch.org/whl/cpu             torch==2.3.1 torchvision==0.18.1 torchaudio==2.3.1
